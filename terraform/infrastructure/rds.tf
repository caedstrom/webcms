...
resource "aws_rds_cluster" "db_encrypt" {
  cluster_identifier = "webcms-encrypt-${var.environment}"
...
resource "aws_rds_cluster_instance" "db_instance_encrypt" {
  count = var.db_instance_count

  identifier     = "webcms-encrypt-${var.environment}-${count.index}"
  instance_class = var.db_instance_type
